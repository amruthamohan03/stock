...
class ExportsydoniaController extends Controller
{
    ...
    {
        $data = [
            'title' => 'Export Sydonia'
        ];

        $this->viewWithLayout('sydonia/exportsydonia', $data);
    }
    ...
    public function validate_mca_refs()
    {
        ...
        try {
            ...
            foreach ($records as $row) {
                ...
                try {
                    $sql = "SELECT COUNT(*) as count FROM exports_t WHERE TRIM(UPPER(mca_ref)) = TRIM(UPPER(:mca_ref))";
        ...
        try {
            ...
            foreach ($rows as $row) {
                ...
                try {
                    ...
                    // Check each field and only add if not empty
                    ...
                    // If no fields to update (only MCA ref provided, no data)
                    ...
                    $sql = "UPDATE exports_t SET " . implode(", ", $updateFields) . " 
