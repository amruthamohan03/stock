<?php
class CGEAController extends Controller
{
    /*
     * INDEX PAGE
     */
    ...
    public function index()
    {
        ...
        $data = [
            'title' => 'CGEA Management',
            'clients' => $clients,
            'banks' => $banks,
            'csrf_token' => $_SESSION['csrf_token']
        ];

        $this->viewWithLayout('advance/cgea', $data);
    }
    ...
    (c.total_amount - IFNULL(e.total_used, 0)) AS total_balance
        ...
        /*
         * GET CLIENT CGEA DETAILS (for view modal)
         */
        if ($action === 'getClientCGEADetails' && $_SERVER['REQUEST_METHOD'] === 'GET') {
            header('Content-Type: application/json');
            ...
            // Get all CGEA records for this client
            $cgeaQuery = "
                SELECT 
                    c.*,
                    b.bank_name,
            ...
            ";
            $cgeaRecords = $db->customQuery($cgeaQuery, [$clientId]);

            // Calculate totals
            $totalAmount = 0;
            $totalUsed = 0;
            $totalBalance = 0;
            foreach ($cgeaRecords as $record) {
            ...
            echo json_encode([
                ...
                'data' => [
                    ...
                    'totals' => [
                        ...
                        'record_count' => count($cgeaRecords)
                    ]
                ]
            ]);
            exit;
        ...
        if ($action === 'statistics' && $_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            try {
                // Total CGEA records
                $totalQuery = "SELECT COUNT(*) as total FROM cgea_t WHERE display = 'Y'";
                $totalRecords = $db->customQuery($totalQuery)[0]['total'] ?? 0;

                // Total amount
                $amountQuery = "SELECT COALESCE(SUM(amount), 0) as total FROM cgea_t WHERE display = 'Y'";
                $totalAmount = $db->customQuery($amountQuery)[0]['total'] ?? 0;

                // Total used amount
                $usedAmountQuery = "SELECT COALESCE(SUM(used_amount), 0) as total FROM cgea_t WHERE display = 'Y'";
                $totalUsedAmount = $db->customQuery($usedAmountQuery)[0]['total'] ?? 0;

                // Total balance amount
                $balanceAmountQuery = "SELECT COALESCE(SUM(balance_amount), 0) as total FROM cgea_t WHERE display = 'Y'";
                ...
                $clientQuery = "
                    ...
                    FROM clients_t cl
                    LEFT JOIN cgea_t c ON cl.id = c.client_id AND c.display = 'Y'
                    WHERE cl.display = 'Y'
                ...
                $bankQuery = "
                    ...
                    LEFT JOIN cgea_t c ON b.id = c.bank_id AND c.display = 'Y'
                    WHERE b.display = 'Y'
         ...
         * GET SINGLE CGEA
         */
        if ($action === 'getCGEA' && $_SERVER['REQUEST_METHOD'] === 'GET') {
            header('Content-Type: application/json');
            ...
            if (!empty($result)) {
                ...
                echo json_encode(['success' => false, 'message' => 'CGEA record not found']);
            }
            exit;
        }
        ...
        if ($action === 'insert' && $_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            try {
                ...
                echo json_encode($insertId
                    ? ['success' => true, 'message' => 'CGEA record added successfully!', 'id' => $insertId]
                    : ['success' => false, 'message' => 'Insert failed.']
                );
            } catch (Exception $e) {
                error_log('CGEA Insert Error: ' . $e->getMessage());
        ...
        if ($action === 'update' && $_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $oldRow = $db->selectData('cgea_t', '*', ['id' => $id]);
            if (empty($oldRow)) {
                echo json_encode(['success' => false, 'message' => 'CGEA record not found']);
                exit;
            }
            ...
            try {
                ...
                echo json_encode([
                    ...
                    'message' => $update ? 'CGEA record updated successfully!' : 'Update failed.'
                ]);
            } catch (Exception $e) {
                error_log('CGEA Update Error: ' . $e->getMessage());
                echo json_encode(['success' => false, 'message' => 'Database error: ' . $e->getMessage()]);
            }
            exit;
        }
        ...
        if ($action === 'deletion' && $_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            echo json_encode([
                ...
                'message' => $delete ? 'CGEA record deleted successfully!' : 'Delete failed.'
            ]);
            exit;
        }

        /*
         * EXPORT CGEA RECORDS LIST TO EXCEL (SIMPLE - ONLY 4 COLUMNS)
         */
        if ($action === 'exportCGEARecordsList' && $_SERVER['REQUEST_METHOD'] === 'GET') {
            // Get current filters
            ...
            $cgeas = $db->customQuery($query, $params);
            ...
            $sheet->setCellValue('A1', 'CGEA RECORDS LIST');
            ...
            foreach ($cgeas as $cgea) {
                $sheet->setCellValue('A' . $row, $cgea['client_code'] ?? 'N/A');
                $sheet->setCellValue('B' . $row, $cgea['bank_name'] ?? 'N/A');
                $sheet->setCellValue('C' . $row, date('d-m-Y', strtotime($cgea['prepayment_date'])));
                $sheet->setCellValue('D' . $row, '$' . number_format($cgea['amount'], 2));
            ...
            // Download
            $filename = 'CGEA_Records_List_' . date('Y-m-d_His') . '.xlsx';
        ...
        /*
         * EXPORT SINGLE CGEA TO EXCEL
         */
        if ($action === 'exportCGEA' && $_SERVER['REQUEST_METHOD'] === 'GET') {
            $id = intval($_GET['id'] ?? 0);
            if ($id <= 0) {
                die('Invalid ID');
            }
            ...
            $query = "
                ...
                WHERE c.id = ?
            ";
            $cgeaData = $db->customQuery($query, [$id]);

            if (empty($cgeaData)) {
                die('CGEA record not found');
            }
            ...
            // Title
            $sheet->mergeCells('A1:D1');
            $sheet->setCellValue('A1', 'CGEA RECORD DETAILS');
            ...
            $fields = [
                'Client Code' => $cgea['short_name'] ?? 'N/A',
                'Bank' => $cgea['bank_name'] ?? 'N/A',
                'Prepayment Date' => date('d-m-Y', strtotime($cgea['prepayment_date'])),
                'Amount' => '$' . number_format($cgea['amount'], 2),
                'Used Amount' => '$' . number_format($cgea['used_amount'], 2),
                'Balance Amount' => '$' . number_format($cgea['balance_amount'], 2)
            ];
            ...
            // Download
            $filename = 'CGEA_' . $cgea['id'] . '_' . date('Y-m-d') . '.xlsx';
            header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            header('Content-Disposition: attachment; filename="' . $filename . '"');
            header('Cache-Control: max-age=0');
        ...
        if ($action === 'exportClientSummary' && $_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            $query = "
                SELECT 
                    ...
                    COALESCE(SUM(c.balance_amount), 0) as total_balance
                FROM clients_t cl
                INNER JOIN cgea_t c ON cl.id = c.client_id AND c.display = 'Y'
            ...
            $sheet->setCellValue('A1', 'CLIENT-WISE CGEA AMOUNT BREAKDOWN & BALANCE SUMMARY');
            $sheet->getStyle('A1')->applyFromArray([
                'font' => ['bold' => true, 'color' => ['rgb' => 'FFFFFF'], 'size' => 14],
                'fill' => ['fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID, 'startColor' => ['rgb' => '667eea']],
                'alignment' => ['horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER]
            ...
            // Download
            $filename = 'Client_CGEA_Summary_' . date('Y-m-d_His') . '.xlsx';
            header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            header('Content-Disposition: attachment; filename="' . $filename . '"');
            header('Cache-Control: max-age=0');
        ...
        /*
         * EXPORT INDIVIDUAL CLIENT CGEA SUMMARY TO EXCEL
         */
        if ($action === 'exportClientCGEASummary' && $_SERVER['REQUEST_METHOD'] === 'GET') {
            $clientId = intval($_GET['client_id'] ?? 0);
            ...
            $query = "
                ...
                ORDER BY c.prepayment_date DESC, c.id DESC
            ";
            $cgeaRecords = $db->customQuery($query, [$clientId]);
            ...
            $spreadsheet = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
            $sheet = $spreadsheet->getActiveSheet();
            $sheet->setTitle('Client CGEA Summary');
            ...
            // Title
            $sheet->mergeCells('A1:F1');
            $sheet->setCellValue('A1', 'CGEA RECORDS FOR CLIENT: ' . $client['short_name'] . ' - ' . $client['company_name']);
            $sheet->getStyle('A1')->applyFromArray([
                'font' => ['bold' => true, 'color' => ['rgb' => 'FFFFFF'], 'size' => 14],
                'fill' => ['fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID, 'startColor' => ['rgb' => '667eea']],
                'alignment' => ['horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER]
            ...
            $summary = [
                'Total Records' => count($cgeaRecords),
            ...
            // Download
            $filename = 'Client_' . $client['short_name'] . '_CGEA_' . date('Y-m-d_His') . '.xlsx';
            header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            header('Content-Disposition: attachment; filename="' . $filename . '"');
            header('Cache-Control: max-age=0');
