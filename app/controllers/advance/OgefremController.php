<?php
class OgefremController extends Controller
{
    /*
     * INDEX PAGE
     */
    ...
    public function index()
    {
        ...
        $data = [
            'title' => 'OGEFREM Management',
            'banks' => $banks,
            'csrf_token' => $_SESSION['csrf_token']
        ];

        $this->viewWithLayout('advance/ogefrem', $data);
    }
...
//echo $totajlQuery;exit;
    ...
    $usedQuery = "
        SELECT 
            SUM(ogefrem_amount) AS total_used
        FROM exports_t
        WHERE display = 'Y'
    ";
    ...
    exit;
        ...
        if ($action === 'statistics' && $_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            try {
                // Total OGEFREM records
                $totalQuery = "SELECT COUNT(*) as total FROM ogefrem_t WHERE display = 'Y'";
                $totalRecords = $db->customQuery($totalQuery)[0]['total'] ?? 0;

                // Total amount
                $amountQuery = "SELECT COALESCE(SUM(amount), 0) as total FROM ogefrem_t WHERE display = 'Y'";
                $totalAmount = $db->customQuery($amountQuery)[0]['total'] ?? 0;

                // Total used amount
                $usedAmountQuery = "SELECT COALESCE(SUM(used_amount), 0) as total FROM ogefrem_t WHERE display = 'Y'";
                $totalUsedAmount = $db->customQuery($usedAmountQuery)[0]['total'] ?? 0;

                // Total balance amount
                $balanceAmountQuery = "SELECT COALESCE(SUM(balance_amount), 0) as total FROM ogefrem_t WHERE display = 'Y'";
                ...
                $bankQuery = "
                    ...
                    LEFT JOIN ogefrem_t c ON b.id = c.bank_id AND c.display = 'Y'
                    WHERE b.display = 'Y'
         ...
         * GET SINGLE OGEFREM
         */
        if ($action === 'getOGEFREM' && $_SERVER['REQUEST_METHOD'] === 'GET') {
            header('Content-Type: application/json');
            ...
            if (!empty($result)) {
                ...
                echo json_encode(['success' => false, 'message' => 'OGEFREM record not found']);
            }
            exit;
        }

        if ($action === 'getogefremDetails' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    header('Content-Type: application/json');

    // Fetch all OGEFREM records (NO CLIENT LOGIC)
    $ogefremQuery = "
        SELECT 
            c.*,
            b.bank_name,
    ...
    $ogefremRecords = $db->customQuery($ogefremQuery);

    // SAFETY: ensure array
    if (!is_array($ogefremRecords)) {
        $ogefremRecords = [];
    }
    ...
    foreach ($ogefremRecords as $record) {
    ...
    echo json_encode([
        'success' => true,
        'data' => [
            'records' => $ogefremRecords,
            'totals' => [
                'total_amount'  => $totalAmount,
                'total_used'    => $totalUsed,
                'total_balance' => $totalBalance,
                'record_count'  => count($ogefremRecords)
            ]
        ]
    ]);
    exit;
        ...
        if ($action === 'insert' && $_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            try {
                ...
                echo json_encode($insertId
                    ? ['success' => true, 'message' => 'OGEFREM record added successfully!', 'id' => $insertId]
                    : ['success' => false, 'message' => 'Insert failed.']
                );
            } catch (Exception $e) {
                error_log('OGEFREM Insert Error: ' . $e->getMessage());
        ...
        if ($action === 'update' && $_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $oldRow = $db->selectData('ogefrem_t', '*', ['id' => $id]);
            if (empty($oldRow)) {
                echo json_encode(['success' => false, 'message' => 'OGEFREM record not found']);
                exit;
            }
            ...
            try {
                ...
                echo json_encode([
                    ...
                    'message' => $update ? 'OGEFREM record updated successfully!' : 'Update failed.'
                ]);
            } catch (Exception $e) {
                error_log('OGEFREM Update Error: ' . $e->getMessage());
                echo json_encode(['success' => false, 'message' => 'Database error: ' . $e->getMessage()]);
            }
            exit;
        }
        ...
        if ($action === 'deletion' && $_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            echo json_encode([
                ...
                'message' => $delete ? 'OGEFREM record deleted successfully!' : 'Delete failed.'
            ]);
            exit;
        }

        /*
         * EXPORT OGEFREM RECORDS LIST TO EXCEL (WITHOUT CLIENT)
         */
        if ($action === 'exportOGEFREMRecordsList' && $_SERVER['REQUEST_METHOD'] === 'GET') {
            // Get current filters
            ...
            $ogefrems = $db->customQuery($query, $params);
            ...
            $sheet->setTitle('OGEFREM Records');
            ...
            $sheet->setCellValue('A1', 'OGEFREM RECORDS LIST');
            ...
            foreach ($ogefrems as $ogefrem) {
                $sheet->setCellValue('A' . $row, $ogefrem['bank_name'] ?? 'N/A');
                $sheet->setCellValue('B' . $row, date('d-m-Y', strtotime($ogefrem['prepayment_date'])));
                $sheet->setCellValue('C' . $row, '$' . number_format($ogefrem['amount'], 2));
            ...
            // Download
            $filename = 'OGEFREM_Records_List_' . date('Y-m-d_His') . '.xlsx';
        ...
        /*
         * EXPORT SINGLE OGEFREM TO EXCEL
         */
        if ($action === 'exportOGEFREM' && $_SERVER['REQUEST_METHOD'] === 'GET') {
            $id = intval($_GET['id'] ?? 0);
            if ($id <= 0) {
                die('Invalid ID');
            }
            ...
            $query = "
                ...
                WHERE c.id = ?
            ";
            $ogefremData = $db->customQuery($query, [$id]);

            if (empty($ogefremData)) {
                die('OGEFREM record not found');
            }

            $ogefrem = $ogefremData[0];
            ...
            $spreadsheet = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
            $sheet = $spreadsheet->getActiveSheet();
            $sheet->setTitle('OGEFREM Details');
            ...
            $sheet->setCellValue('A1', 'OGEFREM RECORD DETAILS');
            ...
            $fields = [
                'Bank' => $ogefrem['bank_name'] ?? 'N/A',
                'Prepayment Date' => date('d-m-Y', strtotime($ogefrem['prepayment_date'])),
                'Amount' => '$' . number_format($ogefrem['amount'], 2),
                'Used Amount' => '$' . number_format($ogefrem['used_amount'], 2),
                'Balance Amount' => '$' . number_format($ogefrem['balance_amount'], 2)
            ];
            ...
            // Download
            $filename = 'OGEFREM_' . $ogefrem['id'] . '_' . date('Y-m-d') . '.xlsx';
            header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            header('Content-Disposition: attachment; filename="' . $filename . '"');
            header('Cache-Control: max-age=0');
