...
<style>
  ...
  #ogefremDetailsTable .btn-sm {
    padding: 3px 8px;
    font-size: 0.75rem;
  }
  /* DataTable Customization */
  #ogefremDetailsTable_wrapper .dataTables_length,
  #ogefremDetailsTable_wrapper .dataTables_filter {
    font-size: 0.85rem;
  }

  #ogefremDetailsTable_wrapper .dataTables_info,
  #ogefremDetailsTable_wrapper .dataTables_paginate {
    font-size: 0.8rem;
  }
  ...
  <div class="page-container">
    <div class="row">
      <div class="col-12">
          ...
          <div class="col-xl-6 col-md-6 mb-3">
            <div class="card stats-card stats-card-1 shadow-sm active" data-filter-type="all">
              <div class="card-body position-relative">
                ...
                <div class="stats-subtitle">All OGEFREM entries</div>
              </div>
            </div>
          </div>
        ...
        <!-- OGEFREM Form Card (NO CLIENT FIELD) -->
        <div class="card shadow-sm mb-4">
          <div class="card-header border-bottom border-dashed d-flex align-items-center justify-content-between">
            <h4 class="header-title mb-0">
              <i class="ti ti-file-text me-2"></i> 
              <span id="formTitle">Add New OGEFREM</span>
            </h4>
          ...
          <div class="card-body">
            <form id="ogefremForm" method="post" novalidate>
              <!-- CSRF Token -->
              <input type="hidden" name="csrf_token" id="csrf_token" value="<?= $csrf_token ?>">
              <input type="hidden" name="ogefrem_id" id="ogefrem_id" value="">
              <input type="hidden" name="action" id="formAction" value="insert">

              <div class="accordion" id="ogefremAccordion">
                
                <!-- OGEFREM INFORMATION -->
                <div class="accordion-item mb-3">
                  <h2 class="accordion-header">
                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#ogefremInfo">
                      <i class="ti ti-file-text me-2"></i> OGEFREM Information
                    </button>
                  </h2>

                  <div id="ogefremInfo" class="accordion-collapse collapse" data-bs-parent="#ogefremAccordion">
              ...
              <div class="row mt-4">
                <div class="col-12 text-end">
                  ...
                  <button type="submit" class="btn btn-primary ms-2" id="submitBtn">
                    <i class="ti ti-check me-1"></i> <span id="submitBtnText">Save OGEFREM</span>
                  </button>
                </div>
              </div>
        ...
        <!-- OGEFREM Records DataTable (WITHOUT CLIENT COLUMN) -->
        <div class="card shadow-sm mb-4">
          <div class="card-header border-bottom border-dashed d-flex align-items-center justify-content-between">
            <h4 class="header-title mb-0"><i class="ti ti-list me-2"></i> OGEFREM Records List</h4>
            <button type="button" class="btn btn-export-all" id="exportOGEFREMRecordsListBtn">
              <i class="ti ti-file-spreadsheet me-1"></i> Export All
            </button>
          </div>
          <div class="card-body">
            <div class="table-responsive">
              <table id="ogefremTable" class="table table-striped table-bordered dt-responsive nowrap w-100">
                <thead>
                  <tr>
                    <th>ID</th>
                    <th>Bank</th>
          ...
          <div class="card-header border-bottom border-dashed d-flex align-items-center justify-content-between">
            <h4 class="header-title mb-0">
              <i class="ti ti-wallet me-2"></i>  Amount Breakdown & Balance Summary
            </h4>
          ...
          <div class="card-body">
            <div class="table-responsive">
              <table id="ogefremDetailsTable" class="table table-striped table-bordered dt-responsive nowrap w-100">
                <thead>
                  <tr>
...
<div class="modal fade" id="viewogefremModal" tabindex="-1" aria-labelledby="viewogefremModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="viewogefremModalLabel">
          <i class="ti ti-eye me-2"></i> OGEFREM Details
        </h5>
...
<script>
  $(document).ready(function () {
    ...
    // Initialize DataTable (WITHOUT CLIENT COLUMN)
    var ogefremTable;
    function initBasicDataTable() {
      if ($.fn.DataTable.isDataTable('#ogefremTable')) {
        $('#ogefremTable').DataTable().destroy();
      }

      ogefremTable = $('#ogefremTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
          url: '<?= APP_URL ?>/ogefrem/crudData/listing',
          type: 'GET',
          data: function(d) {
        ...
        language: {
          ...
          emptyTable: 'No OGEFREM records found',
...
// Initialize SECOND DataTable - Client-wise Summary (WITH VIEW & EXPORT BUTTONS)
    var ogefremDetailsTable;
    function initDetailsDataTable() { 
      if ($.fn.DataTable.isDataTable('#ogefremDetailsTable')) {
        $('#ogefremDetailsTable').DataTable().destroy();
      }

      ogefremDetailsTable = $('#ogefremDetailsTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
  url: '<?= APP_URL ?>/ogefrem/crudData/clientSummary',
  type: 'GET',
  error: function(xhr, error, code) {
    console.error('OGEFREM Summary error:', error, xhr.responseText);
  }
}
,
        columns: [
          ...
          },
          {
            ...
            render: function (data, type, row) {
              return `
                <button class="btn btn-sm btn-view viewClientBtn"  title="View Details">
                  <i class="ti ti-eye"></i>
                </button>
                <button class="btn btn-sm btn-export exportClientBtn"  title="Export to Excel">
                  <i class="ti ti-file-spreadsheet"></i>
                </button>
              `;
            }
    ...
    // Export OGEFREM Records List Button Handler
    $('#exportOGEFREMRecordsListBtn').on('click', function() {
      Swal.fire({
        title: 'Exporting...',
        text: 'Please wait while we prepare your OGEFREM records Excel export',
      ...
      const exportUrl = '<?= APP_URL ?>/ogefrem/crudData/exportOGEFREMRecordsList?bank_filter=' + currentBankFilter + '&date_from=' + currentDateFrom + '&date_to=' + currentDateTo;
    ...
    // Export individual OGEFREM
    $(document).on('click', '.exportBtn', function () {
      const id = $(this).data('id');
      const exportUrl = '<?= APP_URL ?>/ogefrem/crudData/exportOGEFREM?id=' + id;
      
      Swal.fire({
        title: 'Exporting...',
        text: 'Preparing OGEFREM data for export',
        allowOutsideClick: false,
        didOpen: () => {
          Swal.showLoading();
        }
    ...
    $(document).on('click', '.viewClientBtn', function () {
      ...
      $.ajax({
        url: '<?= APP_URL ?>/ogefrem/crudData/getogefremDetails',
        method: 'GET',
        ...
        success: function (res) {
          ...
          if (res.success && res.data) {
            ...
            $('#clientogefremRecordsBody').html(recordsHtml);
            
            // Show modal
            $('#viewogefremModal').modal('show');
          } else {
            Swal.fire('Error', res.message || 'Failed to load ogefrem data', 'error');
          }
        },
        error: function () {
          Swal.close();
          Swal.fire('Error', 'Failed to load ogefrem data. Please try again.', 'error');
        }
      });
    });
    ...
    function updateStatistics() {
      $.ajax({
        url: '<?= APP_URL ?>/ogefrem/crudData/statistics',
        method: 'GET',
        dataType: 'json',
        success: function(res) {
          if (res.success) {
    ...
    $(document).on('click', '.stats-card', function() {
      ...
      if (typeof ogefremTable !== 'undefined') {
        ogefremTable.ajax.reload();
      }
      if (typeof ogefremDetailsTable !== 'undefined') {
        ogefremDetailsTable.ajax.reload();
      }
    });
    ...
    // Form submission
    $('#ogefremForm').on('submit', function (e) {
      e.preventDefault();
      ...
      $.ajax({
        url: '<?= APP_URL ?>/ogefrem/crudData/' + action,
        method: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        ...
        success: function (res) {
          ...
          if (res.success) {
            ...
            resetForm();
            if (typeof ogefremTable !== 'undefined') {
              ogefremTable.ajax.reload(null, false);
            }
            updateStatistics();
          } else {
            Swal.fire({ 
    ...
    function resetForm() {
      ...
      $('#formTitle').text('Add New OGEFREM');
      $('#submitBtnText').text('Save OGEFREM');
      $('#resetFormBtn').hide();
      $('#display').val('Y');
      currentUsedAmount = 0;
      isEditMode = false;
      ...
      // Collapse accordion
      $('#ogefremInfo').collapse('hide');

      $('html, body').animate({ scrollTop: $('#ogefremForm').offset().top - 100 }, 200);
    }
    ...
    $(document).on('click', '.editBtn', function () {
      ...
      $.ajax({
        url: '<?= APP_URL ?>/ogefrem/crudData/getOGEFREM',
        method: 'GET',
        data: { id: id },
        dataType: 'json',
        success: function (res) {
          ...
          if (res.success && res.data) {
            const ogefrem = res.data;

            clearValidationErrors();

            $('#ogefrem_id').val(ogefrem.id);
            $('#formAction').val('update');
            $('#formTitle').text('Edit OGEFREM');
            $('#submitBtnText').text('Update OGEFREM');
            $('#resetFormBtn').show();

            $('#bank_id').val(ogefrem.bank_id || '');
            $('#prepayment_date').val(ogefrem.prepayment_date || '');
            $('#amount').val(ogefrem.amount || '');
            $('#display').val(ogefrem.display || 'Y');

            // Store current used amount
            currentUsedAmount = parseFloat(ogefrem.used_amount || 0);
            isEditMode = true;

            // Open accordion when editing
            $('#ogefremInfo').collapse('show');

            $('html, body').animate({ scrollTop: $('#ogefremForm').offset().top - 100 }, 500);
          } else {
            Swal.fire('Error', res.message || 'Failed to load OGEFREM data', 'error');
          }
        },
    ...
    $(document).on('click', '.deleteBtn', function () {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: '<?= APP_URL ?>/ogefrem/crudData/deletion',
            method: 'POST',
            data: { 
              id: id,
            ...
            error: function (xhr) {
              let errorMsg = 'Failed to delete OGEFREM record';
