...
<style>
  ...
  #cgeaDetailsTable .btn-sm {
    padding: 3px 8px;
    font-size: 0.75rem;
  }

  /* Client Details Modal - CGEA Records Table */
  #clientCGEARecordsTable {
    font-size: 0.8rem;
  }

  #clientCGEARecordsTable thead th {
    background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
    color: white;
    padding: 8px;
    font-weight: 600;
  ...
  #clientCGEARecordsTable tbody td {
    padding: 8px;
  ...
  #cgeaDetailsTable_wrapper .dataTables_length,
  #cgeaDetailsTable_wrapper .dataTables_filter {
    font-size: 0.85rem;
  }

  #cgeaDetailsTable_wrapper .dataTables_info,
  #cgeaDetailsTable_wrapper .dataTables_paginate {
    font-size: 0.8rem;
  }
  ...
  <div class="page-container">
    <div class="row">
      <div class="col-12">
          ...
          <div class="col-xl-4 col-md-6 mb-3">
            <div class="card stats-card stats-card-1 shadow-sm active" data-filter-type="all">
              <div class="card-body position-relative">
                ...
                <div class="stats-subtitle">All CGEA entries</div>
              </div>
            </div>
          </div>
        ...
        <div class="card shadow-sm mb-4">
          <div class="card-header border-bottom border-dashed d-flex align-items-center justify-content-between">
            <h4 class="header-title mb-0">
              ...
              <span id="formTitle">Add New CGEA</span>
            </h4>
          ...
          <div class="card-body">
            <form id="cgeaForm" method="post" novalidate>
              <!-- CSRF Token -->
              <input type="hidden" name="csrf_token" id="csrf_token" value="<?= $csrf_token ?>">
              <input type="hidden" name="cgea_id" id="cgea_id" value="">
              <input type="hidden" name="action" id="formAction" value="insert">

              <div class="accordion" id="cgeaAccordion">
                
                <!-- CGEA INFORMATION -->
                <div class="accordion-item mb-3">
                  <h2 class="accordion-header">
                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#cgeaInfo">
                      <i class="ti ti-file-text me-2"></i> CGEA Information
                    </button>
                  </h2>

                  <div id="cgeaInfo" class="accordion-collapse collapse" data-bs-parent="#cgeaAccordion">
              ...
              <div class="row mt-4">
                <div class="col-12 text-end">
                  ...
                  <button type="submit" class="btn btn-primary ms-2" id="submitBtn">
                    <i class="ti ti-check me-1"></i> <span id="submitBtnText">Save CGEA</span>
                  </button>
                </div>
              </div>
        ...
        <!-- FIRST DataTable - Basic CGEA Records (WITH EXPORT ALL BUTTON) -->
        <div class="card shadow-sm mb-4">
          <div class="card-header border-bottom border-dashed d-flex align-items-center justify-content-between">
            <h4 class="header-title mb-0"><i class="ti ti-list me-2"></i> CGEA Records List</h4>
            <button type="button" class="btn btn-export-all" id="exportCGEARecordsListBtn">
              <i class="ti ti-file-spreadsheet me-1"></i> Export All
            </button>
          </div>
          <div class="card-body">
            <div class="table-responsive">
              <table id="cgeaTable" class="table table-striped table-bordered dt-responsive nowrap w-100">
                <thead>
                  <tr>
                    <th>ID</th>
                    <th>Client</th>
        ...
        <div class="card shadow-sm">
          <div class="card-header border-bottom border-dashed d-flex align-items-center justify-content-between">
            <h4 class="header-title mb-0">
              <i class="ti ti-wallet me-2"></i> Client-wise CGEA Amount Breakdown & Balance Summary
            </h4>
          ...
          <div class="card-body">
            <div class="table-responsive">
              <table id="cgeaDetailsTable" class="table table-striped table-bordered dt-responsive nowrap w-100">
                <thead>
                  <tr>
                    <th>Client Code</th>
                    <th>Client Name</th>
...
<!-- View Client CGEA Details Modal -->
<div class="modal fade" id="viewClientCGEAModal" tabindex="-1" aria-labelledby="viewClientCGEAModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="viewClientCGEAModalLabel">
          <i class="ti ti-eye me-2"></i> Client CGEA Details
        </h5>
      ...
      <div class="modal-body">
        ...
        <!-- CGEA Records Table -->
        <h6 class="mb-3"><i class="ti ti-list me-2"></i> All CGEA Records</h6>
        <div class="table-responsive">
          <table id="clientCGEARecordsTable" class="table table-striped table-bordered w-100">
            <thead>
              <tr>
                <th>ID</th>
                <th>Bank</th>
            ...
            <tbody id="clientCGEARecordsBody">
...
<script>
  $(document).ready(function () {
    ...
    function initBasicDataTable() {
      if ($.fn.DataTable.isDataTable('#cgeaTable')) {
        $('#cgeaTable').DataTable().destroy();
      }

      cgeaTable = $('#cgeaTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
          url: '<?= APP_URL ?>/cgea/crudData/listing',
          type: 'GET',
          data: function(d) {
        ...
        language: {
          ...
          emptyTable: 'No CGEA records found',
    ...
    var cgeaDetailsTable;
    function initDetailsDataTable() {
      if ($.fn.DataTable.isDataTable('#cgeaDetailsTable')) {
        $('#cgeaDetailsTable').DataTable().destroy();
      }

      cgeaDetailsTable = $('#cgeaDetailsTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
          url: '<?= APP_URL ?>/cgea/crudData/clientSummary',
          type: 'GET',
          data: function(d) {
            d.client_filter = currentClientFilter;
          },
    ...
    // View Client CGEA Details Button Handler
    $(document).on('click', '.viewClientBtn', function () {
      ...
      $.ajax({
        url: '<?= APP_URL ?>/cgea/crudData/getClientCGEADetails',
        method: 'GET',
        ...
        success: function (res) {
          ...
          if (res.success && res.data) {
            ...
            $('#clientCGEARecordsBody').html(recordsHtml);
            
            // Show modal
            $('#viewClientCGEAModal').modal('show');
          } else {
            Swal.fire('Error', res.message || 'Failed to load client CGEA data', 'error');
          }
        },
        error: function () {
          Swal.close();
          Swal.fire('Error', 'Failed to load client CGEA data. Please try again.', 'error');
        }
      });
    });

    // Export CGEA Records List Button Handler (NEW - WITH FILTERS)
    $('#exportCGEARecordsListBtn').on('click', function() {
      Swal.fire({
        title: 'Exporting...',
        text: 'Please wait while we prepare your CGEA records Excel export',
        allowOutsideClick: false,
        didOpen: () => {
          Swal.showLoading();
        }
      ...
      const exportUrl = '<?= APP_URL ?>/cgea/crudData/exportCGEARecordsList?client_filter=' + currentClientFilter + '&bank_filter=' + currentBankFilter + '&date_from=' + currentDateFrom + '&date_to=' + currentDateTo;
    ...
    $('#exportClientSummaryBtn').on('click', function() {
      ...
      const exportUrl = '<?= APP_URL ?>/cgea/crudData/exportClientSummary?client_filter=' + currentClientFilter;
      window.location.href = exportUrl;
    ...
    // Export Individual Client CGEA Summary
    $(document).on('click', '.exportClientBtn', function () {
      const clientId = $(this).data('client-id');
      const exportUrl = '<?= APP_URL ?>/cgea/crudData/exportClientCGEASummary?client_id=' + clientId;
      
      Swal.fire({
        title: 'Exporting...',
        text: 'Preparing client CGEA data for export',
        allowOutsideClick: false,
        didOpen: () => {
          Swal.showLoading();
        }
    ...
    // Export individual CGEA
    $(document).on('click', '.exportBtn', function () {
      const id = $(this).data('id');
      const exportUrl = '<?= APP_URL ?>/cgea/crudData/exportCGEA?id=' + id;
      
      Swal.fire({
        title: 'Exporting...',
        text: 'Preparing CGEA data for export',
        allowOutsideClick: false,
        didOpen: () => {
          Swal.showLoading();
        }
    ...
    function updateStatistics() {
      $.ajax({
        url: '<?= APP_URL ?>/cgea/crudData/statistics',
        method: 'GET',
        dataType: 'json',
        success: function(res) {
          if (res.success) {
    ...
    $(document).on('click', '.stats-card', function() {
      ...
      if (typeof cgeaTable !== 'undefined') {
        cgeaTable.ajax.reload();
      }
      if (typeof cgeaDetailsTable !== 'undefined') {
        cgeaDetailsTable.ajax.reload();
      }
    });
    ...
    $('#cgeaForm').on('submit', function (e) {
      ...
      $.ajax({
        url: '<?= APP_URL ?>/cgea/crudData/' + action,
        method: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        ...
        success: function (res) {
          ...
          if (res.success) {
            ...
            resetForm();
            if (typeof cgeaTable !== 'undefined') {
              cgeaTable.ajax.reload(null, false);
            }
            if (typeof cgeaDetailsTable !== 'undefined') {
              cgeaDetailsTable.ajax.reload(null, false);
            }
            updateStatistics();
          } else {
            Swal.fire({ 
    ...
    function resetForm() {
      ...
      $('#formTitle').text('Add New CGEA');
      $('#submitBtnText').text('Save CGEA');
      $('#resetFormBtn').hide();
      $('#display').val('Y');
      currentUsedAmount = 0;
      isEditMode = false;
      ...
      // Collapse accordion
      $('#cgeaInfo').collapse('hide');

      $('html, body').animate({ scrollTop: $('#cgeaForm').offset().top - 100 }, 200);
    }
    ...
    $(document).on('click', '.editBtn', function () {
      ...
      $.ajax({
        url: '<?= APP_URL ?>/cgea/crudData/getCGEA',
        method: 'GET',
        data: { id: id },
        dataType: 'json',
        success: function (res) {
          ...
          if (res.success && res.data) {
            ...
            $('#formTitle').text('Edit CGEA');
            $('#submitBtnText').text('Update CGEA');
            $('#resetFormBtn').show();

            $('#client_id').val(cgea.client_id || '');
            $('#bank_id').val(cgea.bank_id || '');
            $('#prepayment_date').val(cgea.prepayment_date || '');
            $('#amount').val(cgea.amount || '');
            $('#display').val(cgea.display || 'Y');

            // Store current used amount
            currentUsedAmount = parseFloat(cgea.used_amount || 0);
            isEditMode = true;

            // Open accordion when editing
            $('#cgeaInfo').collapse('show');

            $('html, body').animate({ scrollTop: $('#cgeaForm').offset().top - 100 }, 500);
          } else {
            Swal.fire('Error', res.message || 'Failed to load CGEA data', 'error');
          }
        },
        error: function () {
          Swal.close();
          Swal.fire('Error', 'Failed to load CGEA data. Please try again.', 'error');
        }
      });
    });
    ...
    $(document).on('click', '.deleteBtn', function () {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: '<?= APP_URL ?>/cgea/crudData/deletion',
            method: 'POST',
            data: { 
              id: id,
            ...
            error: function (xhr) {
              let errorMsg = 'Failed to delete CGEA record';
