...
<style>
  ...
  #lmcDetailsTable .btn-sm {
    padding: 3px 8px;
    font-size: 0.75rem;
  }

  /* Client Details Modal - lmc Records Table */
  #clientlmcRecordsTable {
    font-size: 0.8rem;
  }

  #clientlmcRecordsTable thead th {
    background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
    color: white;
    padding: 8px;
    font-weight: 600;
  ...
  #clientlmcRecordsTable tbody td {
    padding: 8px;
  ...
  #lmcDetailsTable_wrapper .dataTables_length,
  #lmcDetailsTable_wrapper .dataTables_filter {
    font-size: 0.85rem;
  }

  #lmcDetailsTable_wrapper .dataTables_info,
  #lmcDetailsTable_wrapper .dataTables_paginate {
    font-size: 0.8rem;
  }
...
<div class="page-content">
  <div class="page-container">
    <div class="row">
      <div class="col-12">
          ...
          <div class="col-xl-6 col-md-6 mb-3">
            <div class="card stats-card stats-card-1 shadow-sm active" data-filter-type="all">
              <div class="card-body position-relative">
                ...
                <div class="stats-subtitle">All LMC entries</div>
              </div>
            </div>
          </div>
        ...
        <!-- LMC Form Card (NO CLIENT FIELD) -->
        <div class="card shadow-sm mb-4">
          <div class="card-header border-bottom border-dashed d-flex align-items-center justify-content-between">
            <h4 class="header-title mb-0">
              <i class="ti ti-file-text me-2"></i> 
              <span id="formTitle">Add New LMC</span>
            </h4>
          ...
          <div class="card-body">
            <form id="lmcForm" method="post" novalidate>
              <!-- CSRF Token -->
              <input type="hidden" name="csrf_token" id="csrf_token" value="<?= $csrf_token ?>">
              <input type="hidden" name="lmc_id" id="lmc_id" value="">
              <input type="hidden" name="action" id="formAction" value="insert">

              <div class="accordion" id="lmcAccordion">
                
                <!-- LMC INFORMATION -->
                <div class="accordion-item mb-3">
                  <h2 class="accordion-header">
                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#lmcInfo">
                      <i class="ti ti-file-text me-2"></i> LMC Information
                    </button>
                  </h2>

                  <div id="lmcInfo" class="accordion-collapse collapse" data-bs-parent="#lmcAccordion">
              ...
              <div class="row mt-4">
                <div class="col-12 text-end">
                  ...
                  <button type="submit" class="btn btn-primary ms-2" id="submitBtn">
                    <i class="ti ti-check me-1"></i> <span id="submitBtnText">Save LMC</span>
                  </button>
                </div>
              </div>
        ...
        <!-- LMC Records DataTable (WITHOUT CLIENT COLUMN) -->
        <div class="card shadow-sm mb-4">
          <div class="card-header border-bottom border-dashed d-flex align-items-center justify-content-between">
            <h4 class="header-title mb-0"><i class="ti ti-list me-2"></i> LMC Records List</h4>
            <button type="button" class="btn btn-export-all" id="exportLMCRecordsListBtn">
              <i class="ti ti-file-spreadsheet me-1"></i> Export All
            </button>
          </div>
          <div class="card-body">
            <div class="table-responsive">
              <table id="lmcTable" class="table table-striped table-bordered dt-responsive nowrap w-100">
                <thead>
                  <tr>
                    <th>ID</th>
                    <th>Bank</th>
 ...
 <!-- SECOND DataTable - Client-wise Amount Breakdown Summary (WITH VIEW & EXPORT) -->
        <div class="card shadow-sm">
          ...
          <div class="card-body">
            <div class="table-responsive">
              <table id="lmcDetailsTable" class="table table-striped table-bordered dt-responsive nowrap w-100">
                <thead>
                  <tr>
...
<div class="modal fade" id="viewClientlmcModal" tabindex="-1" aria-labelledby="viewClientlmcModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="viewClientlmcModalLabel">
          <i class="ti ti-eye me-2"></i> LMC Details
        </h5>
      ...
      <div class="modal-body">
        ...
        <!-- lmc Records Table -->
        <!-- <h6 class="mb-3"><i class="ti ti-list me-2"></i> All LMC Records</h6> -->
        <!-- <div class="table-responsive">
          <table id="clientlmcRecordsTable" class="table table-striped table-bordered w-100">
            <thead>
              <tr>
                <th>ID</th>
                <th>Bank</th>
...
<script>
  $(document).ready(function () {
    ...
    function initBasicDataTable() {
      if ($.fn.DataTable.isDataTable('#lmcTable')) {
        $('#lmcTable').DataTable().destroy();
      }

      lmcTable = $('#lmcTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
          url: '<?= APP_URL ?>/lmc/crudData/listing',
          type: 'GET',
          data: function(d) {
        ...
        language: {
          ...
          emptyTable: 'No LMC records found',
    ...
    function initDetailsDataTable() { 
      if ($.fn.DataTable.isDataTable('#lmcDetailsTable')) {
        $('#lmcDetailsTable').DataTable().destroy();
      }

      lmcDetailsTable = $('#lmcDetailsTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
  url: '<?= APP_URL ?>/lmc/crudData/clientSummary',
  type: 'GET',
  error: function(xhr, error, code) {
    console.error('lmc Summary error:', error, xhr.responseText);
  }
}
,
        columns: [
    ...
    $(document).on('click', '.viewClientBtn', function () {
      ...
      $.ajax({
        url: '<?= APP_URL ?>/lmc/crudData/getlmcDetails',
        method: 'GET',
        ...
        success: function (res) {
          ...
          if (res.success && res.data) {
            ...
            $('#clientlmcRecordsBody').html(recordsHtml);
            
            // Show modal
            $('#viewClientlmcModal').modal('show');
          } else {
            Swal.fire('Error', res.message || 'Failed to load LMC data', 'error');
          }
        },
        error: function () {
          Swal.close();
          Swal.fire('Error', 'Failed to load LMC data. Please try again.', 'error');
        }
      });
    });

    // Export LMC Records List Button Handler
    $('#exportLMCRecordsListBtn').on('click', function() {
      Swal.fire({
        title: 'Exporting...',
        text: 'Please wait while we prepare your LMC records Excel export',
        allowOutsideClick: false,
        didOpen: () => {
          Swal.showLoading();
        }
      ...
      const exportUrl = '<?= APP_URL ?>/lmc/crudData/exportLMCRecordsList?bank_filter=' + currentBankFilter + '&date_from=' + currentDateFrom + '&date_to=' + currentDateTo;
    ...
    // Export individual LMC
    $(document).on('click', '.exportBtn', function () {
      const id = $(this).data('id');
      const exportUrl = '<?= APP_URL ?>/lmc/crudData/exportLMC?id=' + id;
      
      Swal.fire({
        title: 'Exporting...',
        text: 'Preparing LMC data for export',
        allowOutsideClick: false,
        didOpen: () => {
          Swal.showLoading();
        }
    ...
    function updateStatistics() {
      $.ajax({
        url: '<?= APP_URL ?>/lmc/crudData/statistics',
        method: 'GET',
        dataType: 'json',
        success: function(res) {
          if (res.success) {
    ...
    $(document).on('click', '.stats-card', function() {
      ...
      if (typeof lmcTable !== 'undefined') {
        lmcTable.ajax.reload();
      }
      if (typeof lmcDetailsTable !== 'undefined') {
        lmcDetailsTable.ajax.reload();
      }
    });
    ...
    $('#lmcForm').on('submit', function (e) {
      ...
      $.ajax({
        url: '<?= APP_URL ?>/lmc/crudData/' + action,
        method: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        ...
        success: function (res) {
          ...
          if (res.success) {
            ...
            resetForm();
            if (typeof lmcTable !== 'undefined') {
              lmcTable.ajax.reload(null, false);
            }
            updateStatistics();
          } else {
            Swal.fire({ 
    ...
    function resetForm() {
      ...
      $('#formTitle').text('Add New LMC');
      $('#submitBtnText').text('Save LMC');
      $('#resetFormBtn').hide();
      $('#display').val('Y');
      currentUsedAmount = 0;
      isEditMode = false;
      ...
      // Collapse accordion
      $('#lmcInfo').collapse('hide');

      $('html, body').animate({ scrollTop: $('#lmcForm').offset().top - 100 }, 200);
    }
    ...
    $(document).on('click', '.editBtn', function () {
      ...
      $.ajax({
        url: '<?= APP_URL ?>/lmc/crudData/getLMC',
        method: 'GET',
        data: { id: id },
        dataType: 'json',
        success: function (res) {
          ...
          if (res.success && res.data) {
            ...
            $('#formTitle').text('Edit LMC');
            $('#submitBtnText').text('Update LMC');
            $('#resetFormBtn').show();

            $('#bank_id').val(lmc.bank_id || '');
            $('#prepayment_date').val(lmc.prepayment_date || '');
            $('#amount').val(lmc.amount || '');
            $('#display').val(lmc.display || 'Y');

            // Store current used amount
            currentUsedAmount = parseFloat(lmc.used_amount || 0);
            isEditMode = true;

            // Open accordion when editing
            $('#lmcInfo').collapse('show');

            $('html, body').animate({ scrollTop: $('#lmcForm').offset().top - 100 }, 500);
          } else {
            Swal.fire('Error', res.message || 'Failed to load LMC data', 'error');
          }
        },
    ...
    $(document).on('click', '.deleteBtn', function () {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: '<?= APP_URL ?>/lmc/crudData/deletion',
            method: 'POST',
            data: { 
              id: id,
            ...
            error: function (xhr) {
              let errorMsg = 'Failed to delete LMC record';
